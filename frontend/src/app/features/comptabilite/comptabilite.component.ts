  ...
  calculateStatistics(): void {
    ...
    this.orders.forEach(order => {
      order.orderItems.forEach(item => {
        const pizzaName = item.half1.pizza.name + (item.half2 ? '/' + item.half2.pizza.name : '');
        ...
        const pizzaRevenue = item.quantity * Math.max(item.half1.pizza.price, item.half2 ? item.half2.pizza.price : 0);
